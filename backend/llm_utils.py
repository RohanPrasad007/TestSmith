...
def generate_paper():
    ...
    """
    client = genai.Client(
        api_key=os.getenv("API_KEY"),  # Read API key from .env file
    )
        ...
        types.Content(
            ...
            parts=[
                types.Part.from_text(
                    text="""give me a jee question paper Physics: 20 Multiple-Choice Questions (MCQs) and 5 Numerical Value Questions (NVQs)
Chemistry: 20 MCQs and 5 NVQs
Mathematics: 20 MCQs and 5 NVQs"""
                ),
            ],
    ...
    try:
        question_paper = json.loads(cleaned_text)
