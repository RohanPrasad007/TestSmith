    ...
    backdrop-blur-md border-none outline-none blur-[150px] md:blur-[200px] lg:blur-[300px] z-0" />
            ...
            <div className="min-h-screen bg-opacity-70 text-white py-6 md:py-8 lg:py-12 relative z-10">
                <div className="container mx-auto px-4">
                    <h1 className="text-2xl md:text-3xl font-bold mb-8 md:mb-10 lg:mb-14 text-center">Review Your Answers</h1>
                        ...
                        <div className="flex overflow-x-auto scrollbar-hide border-b border-gray-700 w-full md:w-[80%] lg:w-[60%] mx-auto justify-between">
                            {subjects.map(subject => {
                                ...
                                ${isActive ? 'border-blue-500 text-blue-400' : 'border-transparent hover:text-gray-300'}`}
                                    >
                                        <span className="truncate max-w-[80px] md:max-w-full">{subject}</span>
                                        <div className="flex items-center gap-1 text-xs">
                                            <span className="bg-green-600 px-1 md:px-2 py-0.5 rounded">{stats.correct}</span>
                                            <span className="bg-red-600 px-1 md:px-2 py-0.5 rounded">{stats.incorrect}</span>
                                            <span className="bg-gray-700 px-1 md:px-2 py-0.5 rounded">{stats.total - stats.attempted}</span>
                                        </div>
                                    </button>
                                );
                            })}
                        ...
                        <div className="bg-gray-800/50 p-3 md:p-4 lg:p-6 rounded-lg shadow-lg w-full md:w-[95%] lg:w-[90%] max-w-[900px] mx-auto">
                            <div className="flex flex-col sm:flex-row justify-between mb-3 md:mb-4 gap-2 sm:gap-0">
                                <span className="bg-gray-700/40 px-2 md:px-3 py-2 md:py-4 rounded text-sm md:text-base">
                                    {selectedSubject} - Question {currentQuestion + 1} of {filteredQuestions.length}
                                </span>
                                <span className="bg-gray-700/40 px-2 md:px-3 py-2 md:py-4 rounded text-sm md:text-base">
                            ...
                            <h2 className="text-xl md:text-2xl lg:text-3xl my-4 md:my-6">{subjectQuestion.question}</h2>
                            ...
                            {subjectQuestion.type === 'MCQ' && subjectQuestion.options && (
                                <ul className="space-y-2">
                                    {subjectQuestion.options.map((option, index) => {
                                        ...
                                        let optionClass = "p-3 md:p-4 pl-3 md:pl-4 rounded-md text-sm md:text-base";
                            ...
                            {subjectQuestion.type === 'Numerical' && (
                                <div className="mt-4">
                                    <label className="block text-gray-300 mb-3 md:mb-4 text-base md:text-lg">Your numerical answer:</label>
                                    <div className="flex gap-2 mb-4">
                                        <input
                                            type="number"
                                            className={`p-2 md:p-3 rounded-md flex-grow text-white bg-gray-700/40 ${isAnswerCorrect(subjectQuestion, userAnswers[`${selectedSubject}-${currentQuestion}`])
