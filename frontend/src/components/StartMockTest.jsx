...
const StartMockTest = () => {
    ...
    try {
      ...
      const availableQuestions = data.questions.filter(question => {
        ...
        const isAnswered = Object.keys(userAnswers).some(key => {
          ...
          const subjectQuestions = questions.filter(q => q.subject === answerSubject);
      ...
      showToast.error("Failed to load JEE questions from server. Please try again.");
  ...
  const fetchNEETQuestions = async () => {
    ...
    try {
      ...
      showToast.error("Failed to load NEET questions from server. Please try again.");
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-[#0A0A0B] via-[#110D1A] to-[#090015] overflow-hidden relative">
      ...
      <div className="w-[70vw] h-[70vw] max-w-[800px] max-h-[700px] absolute right-[-10%] sm:right-[0%] md:right-[5%] lg:right-[12%] top-[-20%] rounded-full 
        ...
        {!localQuestions.length && !loading && (
          <>
            ...
            <div className="flex justify-between items-center mb-4 sm:mb-6 md:mb-8 max-w-7xl mx-auto bg-white/5 p-2 px-3 sm:px-4 rounded-full">
              <div>
                <img
                  src={user.providerData?.[0]?.photoURL || "/default-profile.png"}
                  alt="User"
          ...
          {loading ? (
            <div className="w-full h-screen flex justify-center items-center flex-col">
              <div className='flex flex-col text-center mt-[10px]'>
                <p className="text-2xl font-semibold ">Your practice test is currently building...</p>
                <p className="mt-2 sm:mt-3 md:mt-4 text-3xl sm:text-3xl md:text-5xl lg:text-6xl font-semibold bg-gradient-to-r from-white via-purple-500 to-purple-900 text-transparent bg-clip-text">Till then, meet our friend Smith!</p>
              </div>
              <spline-viewer
            ...
            localQuestions.length > 0 && (
              <div className="w-full h-full">
                <ReadyForExam setReadyForExam={setReadyForExam} user={user} examType={examType} />
              </div>
            )
          )}
        </div>
